...
import org.scalatest.{FreeSpecLike, Inside}

// FIXME - some bug with classloader so that it doesn't pick up the default application.conf
class TodoStoreActorTest extends ScalaTestWithActorTestKit("""
    include "application.conf"

    akka.actor.serialize-messages = on
  """.stripMargin) with FreeSpecLike with Inside {
  ...
  "TodoStore" - {
    "should be able to receive a new todo" in {
      val res = store ? (
          replyTo => CommandReplyMessage(replyTo, TodoCommand.AddTodo(NewTodo("Wash dishes", completed = false)))
      )

      inside(res.futureValue) {
        case EventReply(u: TodoEvent.TodoUpdated) => u.todo.title should equal("Wash dishes")
      }
    }
  }
